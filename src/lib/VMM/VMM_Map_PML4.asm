VMM_Map_PML4:
  ...
  mov r11, [rcx + r8 * 8]                              ; entry from pml4
                                                       ; 8 - is length of entry
  
  test r11, 0x1                                        
  jz VMM_Map_PML4_Alloc_and_create_pdpt                ; if P != 1
  
  VMM_Map_PML4_PDPT_Entry_exists:
    mov r9, [VMM_4KB_Page_Entry_Mask]
    mov r10, r11                                       ; use entry
    and r10, r9                                        ; phys address PDPT
    jmp VMM_Map_PML4_Get_PDPT_index
  
  VMM_Map_PML4_Alloc_and_create_pdpt:                  ; if P != 1
    mov rax, 1
    mov rbx, 0xff
    push r8
    ...
    pop r8
    cmp rbx, 0x0
    jne VMM_Map_PML4_Allocation_failed
    ...
    mov rcx, [rsp + 8]                                 ; restore values, PML4 phys addres
    ...
    VMM_Map_PML4_Alloc_and_create_pdpt_loop:
      cmp rcx, 512 * 8
      je VMM_Map_PML4_Get_PDPT_index
      
      mov qword [r10 + rcx], 0x0                       ; set 0 in all entries
      
      add rcx, 8
      jmp VMM_Map_PML4_Alloc_and_create_pdpt_loop
  
  
  VMM_Map_PML4_Get_PDPT_index:                         ; r10 - phys address pdpt table
    mov r8, rax
    ...
    test r9, 0x1                                       
    jnz VMM_Map_PML4_Entry_In_PDPT_exists              ; if P != 0
    
    VMM_Map_PML4_Entry_In_PDPT_not_exists:
      test rdx, 0b100_0000_0000
      jz VMM_Map_PML4_Alloc_and_create_pd              ; if 1Gb Page == 0
      
      VMM_Map_PML4_Create_1Gb_Page_Entry:
	...
	ret
      ...
      VMM_Map_PML4_Alloc_and_create_pd:
        ...
        xor rcx, rcx                                   ; init counter
        VMM_Map_PML4_Alloc_and_create_pd_loop:
          cmp rcx, 512 * 8
          je VMM_Map_PML4_Get_PD_index
      
          mov qword [r12 + rcx], 0x0                   ; set 0 in all entries
      
          add rcx, 8
          jmp VMM_Map_PML4_Alloc_and_create_pd_loop
        
        jmp VMM_Map_PML4_Get_PD_index
        
    VMM_Map_PML4_Entry_In_PDPT_exists:
      mov rdx, [rsp]                                   ; restore values, attributes
      test rdx, 0b100_0000_0000
      jnz VMM_Map_PML4_Entry_In_PDPT_exists_error      ; if 1Gb Page != 1
      
      VMM_Map_PML4_Entry_In_PDPT_exists_not_error:
        mov r11, [VMM_4KB_Page_Entry_Mask]
        mov r12, r9                                    ; use entry from pdpt
        and r12, r11                                   ; final phys address pd
        jmp VMM_Map_PML4_Get_PD_index
      
      VMM_Map_PML4_Entry_In_PDPT_exists_error:
        pop rdx
        pop rcx
        pop rbx
        pop rax
  ...
  VMM_Map_PML4_Get_PD_index:                           ; r12 - phys address pd table
    mov r8, rax
    ...
    test r9, 0x1
    jnz VMM_Map_PML4_Get_PD_Entry_Exists               ; if P != 0
    
    VMM_Map_PML4_Get_PD_Entry_Not_Exists:
      mov rdx, [rsp]                                   ; restore values, attributes
      test rdx, 0b10_0000_0000
      jz VMM_Map_PML4_Alloc_and_create_pt              ;if 2Mb Page != 1
      
      VMM_Map_PML4_Create_2Mb_Page_Entry:
        mov r9, 0b1111_1000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0000_0001_1001_0001_1110
        and rdx, r9                                    ; clear other bits
	...
	ret
      ...
      VMM_Map_PML4_Alloc_and_create_pt:
        mov rax, 1                                     ; count pages
        mov rbx, 0xff                                  ; type pages
        push r12
        push r10
        ...
        xor rcx, rcx                                   ; init counter
        VMM_Map_PML4_Alloc_and_create_pt_loop:
          cmp rcx, 512 * 8
          je VMM_Map_PML4_Get_PT_index
      
          mov qword [r13 + rcx], 0x0                   ; set 0 in all entries
      
          add rcx, 8
          jmp VMM_Map_PML4_Alloc_and_create_pt_loop
        
        jmp VMM_Map_PML4_Get_PT_index
        
  
    VMM_Map_PML4_Get_PD_Entry_Exists:
      mov rdx, [rsp]                                   ; restore values, attributes
      test rdx, 0b10_0000_0000
      jnz VMM_Map_PML4_Get_PD_Entry_Exists_Error       ; if 2Mb Page != 0
      
      VMM_Map_PML4_Get_PD_Entry_Exists_Not_Error:
        mov r13, r9                                    ; use entry from pd
        mov r9, [VMM_4KB_Page_Entry_Mask]
        and r13, r9                                    ; final phys address pt
        jmp VMM_Map_PML4_Get_PT_index
        
      VMM_Map_PML4_Get_PD_Entry_Exists_Error:
        pop rdx
        pop rcx
        pop rbx
        pop rax
  ...
  VMM_Map_PML4_Get_PT_index:                           ; r13 - phys address pt table
    mov r8, rax
    ...
    test r9, 0x1
    jnz VMM_Map_PML4_Get_PT_Error                      ; if P == 1
    VMM_Map_PML4_Get_PT_Entry_Not_Exists:
  ...
  VMM_Map_PML4_Allocation_failed:
    pop rdx
    pop rcx
    pop rbx
    pop rax
